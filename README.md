...
- [`1. Validation - Records.sql`](sql/1.%20Validation%20-%20Records.sql) checks the row count of every table using CTEs.
- [`2. Validation - Values.sql`](sql/2.%20Validation%20-%20Values.sql) checks that total sales, purchases, excise taxes, beginning inventory, and ending inventory all matched what the CIO reported.
...
- [`3. Excise Tax Calc - Spirits.sql`](sql/3.%20Excise%20Tax%20Calc%20-%20Spirits.sql) and [`3. Excise Tax Calc - Wine.sql`](sql/3.%20Excise%20Tax%20Calc%20-%20Wine.sql) return every brand where the actual and calculated excise tax do not match.
- [`4. ExciseTaxTotal Spirits.sql`](sql/4.%20ExciseTaxTotal%20Spirits.sql) and [`4. ExciseTaxTotal Wine.sql`](sql/4.%20ExciseTaxTotal%20Wine.sql) roll those discrepancies up into a total dollar difference for each classification.
...
- [`5. Company Analysis.sql`](sql/5.%20Company%20Analysis.sql) calculates COGS, gross margin, inventory turnover (ITR), and days inventory outstanding (DIO) for the company as a whole.
- [`6. TopBottom Brand Analysis.sql`](sql/6.%20TopBottom%20Brand%20Analysis.sql) breaks the same metrics out by brand and returns the top and bottom 6 by gross margin.
- [`7. GrossMarginbyMonth.sql`](sql/7.%20GrossMarginbyMonth.sql) tracks gross margin by month for those same top and bottom brands to check for seasonality.
